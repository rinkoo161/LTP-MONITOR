...
def _smart_money_to_markers(sm):
    ...
    signals on charts"). All markers land at "now" (raw unix epoch;
    the frontend applies the same toLwTime() IST shift it already uses
    for candles, so this stays consistent with however that's handled)
    — these are current-state flags recomputed each TechnicalAgent
    cycle, not a historical time series with a per-event timestamp to
    place them at individually. Capped at 8 total so a very noisy
    chain doesn't clutter the chart."""
    ...
    **FastAPI WebSocket Server** -> TradingView Lightweight Charts.
    ...
    try:
        ...
        while True:
            ...
            #   signals: Feature #4's smart_money engine output, inside
            #     analysis:{symbol} (refreshed every ~60s by
            #     TechnicalAgent) — converted into Lightweight Charts
            #     marker objects. Throttled independently to once every
            #     10s (smart-money events don't need per-second
            #     granularity and this avoids re-deriving markers on
            #     every loop iteration for no reason).
            levels = pilot.bus.get(f"levels:{symbol}")
            if levels and levels != last_levels_sent:
                await websocket.send_json({"type": "levels", "levels": levels})
            ...
            if time.time() - last_signals_ts > 10:
                ...
                if sm:
                    markers = _smart_money_to_markers(sm)
                    if markers:
                        await websocket.send_json({"type": "signals",
                                                   "markers": markers})
