...
class DhanWebsocketClient:
    ...
    def subscribe_more(self, symbol, security_id):
        """Add an instrument to an already-open connection. Returns
        True if the request was actually sent, False if it was
        silently skipped (not connected yet, or bad segment) — callers
        MUST check this before treating the instrument as subscribed,
        since MarketFeed.subscribe_symbols() does not queue a request
        made before the connection is up; it just drops it."""
        segment = SEGMENT_FOR_SYMBOL.get(symbol.upper())
