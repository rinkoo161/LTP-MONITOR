...
def scenario_single_candle_drop(symbol, drop_points, capital, leg="CE",
    ...
    # translate the spot shock into a premium shock via delta approx
    direction = -1 if leg == "CE" else +1   # CE hurt by a drop, PE by a rise
    premium_move = direction * drop_points * DELTA_APPROX
    ...
    return {
        "scenario": f"{symbol} {leg} — {drop_points}pt single-candle shock",
        "qty": qty, "lots": n_lots,
