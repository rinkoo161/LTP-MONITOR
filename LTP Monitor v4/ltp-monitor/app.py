...
def api_journal_shadow(limit: int = 100):
    """Every signal decision, approved or rejected, with resolution
    for rejected ones: did it hit target or stoploss anyway?"""
    import os as _os, json as _j
    if not _os.path.exists(agents.SHADOW_PATH):
        return {"signals": []}
    lines = open(agents.SHADOW_PATH).readlines()[-limit:]
    out = []
    for l in reversed(lines):
        try:
            out.append(_j.loads(l))
        except Exception:
            continue
    ...
    return {"signals": out,
            "stats": {"total": len(out),
                      ...
                      "risk_agent_accuracy_pct": round(
                          hit_sl / len(rejected_resolved) * 100, 1)
                          if rejected_resolved else None}}


@app.get("/api/journal")
def api_journal(days: int = 30):
