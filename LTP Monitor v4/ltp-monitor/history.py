...
def sync_day_chain(get_chain, dhan, symbol, log=print, progress=lambda m: None):
    ...
    # nothing to archive (weekends/holidays) — bail out fast
    seg_kind, sec = SEG[symbol]
    try:
        probe = dhan._intraday_range(symbol, "1", day, day)
    except Exception:
        probe = []
    if not probe:
    ...
    for row in chain["rows"]:
        for leg in ("ce", "pe"):
            ...
            try:
                time.sleep(1.2)
                data = dhan._intraday_range_sid(sid, "1", day, day)
                if data:
    ...
    log_sync(symbol, f"day chain {n_ok} legs")
