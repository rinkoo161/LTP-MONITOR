...
class RiskAgent(Agent):
    ...
    def evaluate(self, job):
        ...
        if profit_target > 0:
            check(self.daily_pnl < profit_target,
                  f"daily profit target reached (₹{self.daily_pnl:.0f} ≥ ₹{profit_target:.0f}) "
                  f"— locking in today's gain, no new positions")
...
class ExecutionAgent(Agent):
    ...
    def _monitor_spreads(self):
        ...
        for sid, sp in list(spreads.items()):
            ...
            lock_trigger = sp["profit_target"] * cfg.get("spread_profit_lock_trigger_pct", 50) / 100
            if pnl_ps >= lock_trigger:
                candidate_floor = round(pnl_ps * cfg.get("spread_profit_lock_pct", 60) / 100, 2)
