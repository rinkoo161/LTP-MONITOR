...
class RegimeAgent(Agent):
    ...
    def _classify(self, sym, d):
        ...
        self.bus.set(f"pa_candles:{sym}", None)  # set below after all fetched
        c15 = self._fetch_candles(d, sym, "15")
        c1 = self._fetch_candles(d, sym, "1")
        self.bus.set(f"pa_candles:{sym}", {"c1": c1, "c5": c5, "ts": time.time()})
            ...
            for sym in syms:
                try:
                    ...
                    history.sync_day_chain(self.ctx["get_chain"], dhan, sym,
                                           log=lambda m: None, progress=prog)
                except Exception as e:
                    self.bus.log(self.name, f"sync {sym}: {str(e)[:60]}")
    ...
    def cycle(self):
        ...
        for sym in self.bus.get("symbols", []):
            ...
            for name in enabled:
                ...
                ev = pa.evaluate(name, pack["c1"], pack["c5"], None,
                ...
                sig = {"signal": "BUY_CE" if ev["dir"] > 0 else "BUY_PE",
                       ...
                       # Retuned for realistic intraday option moves (was
                       # 25% risk / 50% / 100% targets — unrealistically
                       # wide). New: 15% risk, 30%/40% targets. Kept
                       # target1 at exactly 2x risk (rr=2.0) because the
                       # RiskAgent's risk-reward gate requires rr>=1.95 —
                       # a naive 20%/25% version was tried first and
                       # would have produced rr=1.25, silently rejecting
                       # every single PA signal from ever executing.
                       "stoploss": round(entry * 0.85, 1),
                       "target1": round(entry * 1.30, 1),
                       "target2": round(entry * 1.40, 1),
        ...
        self.summary = " · ".join(fired) if fired else \
            f"scanning {len(enabled)} setups across symbols"
